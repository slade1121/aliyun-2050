...
public abstract class BIotProxyBase extends BComponent {
    ...
    /*@ $com.phoenixcontact.AliyunIotMqtt.proxy.BIotProxyBase(3750764269)1.0$ @*/
    /* Generated Wed Apr 06 17:46:00 CST 2022 by Slot-o-Matic (c) Tridium, Inc. 2012 */
...
////////////////////////////////////////////////////////////////
     ...
     * @see #IotNotify
     */
    public void IotNotify() {
        invoke(IotNotify, null, null);
    }
...
////////////////////////////////////////////////////////////////
    ...
    public static BControlPoint CREATE(IotProperty config) {
        if (config.getDataType().equals("int") && config.getAccessMode() == AccessMode.rw) {
        ...
        if (config.getDataType().equals("int") && config.getAccessMode() == AccessMode.r) {
            BNumericPoint point = new BNumericPoint();
            BIotNumericPointProxy proxy = new BIotNumericPointProxy();
            proxy.applyParameters(config);
            point.add("iotProxy", proxy);
            return point;
        }
        if (config.getDataType().equals("bool") && config.getAccessMode() == AccessMode.rw) {
        ...
        if (config.getDataType().equals("bool") && config.getAccessMode() == AccessMode.r) {
            BBooleanPoint point = new BBooleanPoint();
            BIotBooleanPointProxy proxy = new BIotBooleanPointProxy();
            proxy.applyParameters(config);
            point.add("iotProxy", proxy);
            return point;
        }
